...
description: "Use when supporting Cybersecurity customer-service workflows with Black Duck via the blackduck MCP server. For user re-activation, require a corporate email or LDAP username, confirm the identity via the identity directory MCP server, look up the account starting from /api/dormant-users (then fall back to /api/users if needed), and if active=false update the user via HTTP PUT to set active=true using blackduck_users_update. For offboarding cleanup, if a user is dormant for >1 year and is NOT found in the identity directory, suggest deactivation (active=false); after the operator confirms via a manual Slack search, deactivate the Black Duck user via blackduck_users_update. Triggers on: Black Duck, blackduck, dormant users, activate user, reactivate user, enable user, deactivate user, disable user, delete user, remove user, offboarding, user groups, project groups."
license: "Apache-2.0"
compatibility: "Requires network access to the Black Duck instance and a centrally hosted identity directory MCP server. Write operations (e.g., blackduck_users_update and blackduck_project_delete_*) are only available when the MCP bearer token was authorized with access_mode=read_write; otherwise write tools are not registered."
metadata:
  version: "0.2.6"
allowed-tools:
  - "Read"
...
## Connectivity preflight (VPN / private network)
Black Duck upstream APIs (and this MCP server) may live on private-network infrastructure. **Before** invoking any tool below, confirm the network path is available.
...
## Required input (corporate identity)
For enable/reactivation and other standard support requests, require **one** of:
- `<EMAIL>` ending in `@example.com` (case-insensitive)
- `<LDAP_USERNAME>` (example: `mdesales`)

For offboarding cleanup (deactivating long-dormant accounts), you may start from the Black Duck record (dormant list or a `user_id`) and then check the identity directory using the identity fields on the Black Duck user.

For enable/reactivation: do not proceed to Black Duck until the identity is confirmed in the identity directory.

## Step 1 — Confirm the identity in the identity directory (required for enable/reactivation)
Use the identity directory MCP server as the source of truth:
- If the operator provides `<EMAIL>`: call `directory_get_user_by_email` with `email: "<EMAIL>"`
- If the operator provides `<LDAP_USERNAME>`: call `directory_get_user` with `username: "<LDAP_USERNAME>"`

If no user is found:
- For enable/reactivation: stop and ask the operator to double-check the email/username.
- For offboarding cleanup: continue — directory not-found is part of the removal decision (see "Offboarding cleanup" below).

If a directory user is found, capture the canonical values:
- `ldap_username` (directory `username`)
- `email` (directory `email`, normalize to lowercase)
...
1) If the dormant record includes `email`, match it (case-insensitive) against the known email (the identity directory if available; otherwise the operator-provided `<EMAIL>`).
2) Match `externalUserName` / `userName` / `username` (case-insensitive) against the known username (the identity directory `ldap_username` if available; otherwise the operator-provided `<LDAP_USERNAME>`).
...
Selection rules (in priority order):
1) Match the `email` field (case-insensitive) against the known email (the identity directory if available; otherwise the operator-provided `<EMAIL>`).
2) If no email match, match `userName` or `externalUserName` (case-insensitive) against the known username (the identity directory `ldap_username` if available; otherwise the operator-provided `<LDAP_USERNAME>`).
...
### 1) Resolve a user by corporate email or LDAP username
Prompt:
- "Find the Black Duck user for '<EMAIL>' (or '<LDAP_USERNAME>') and return their key properties."

Expected tool flow:
1) Confirm identity in the identity directory (Step 1 above) → get `{ldap_username, email}`
...
Expected tool flow:
1) Confirm identity in the identity directory → `{ldap_username, email}`
...
## Offboarding cleanup — deactivating long-dormant users not found in the identity directory
Sometimes dormant Black Duck accounts belong to employees/contractors who have left the company. If the account has been dormant for **more than 1 year** and the user is **not found in the identity directory**, suggest disabling the account (set `active=false`).
...
Scope guardrails:
- Do NOT deactivate users just because the identity directory is unreachable (network/auth errors) — only proceed when identity-directory tools are functioning.
...
### Step B — Verify the user is NOT in the identity directory
Attempt all applicable identity-directory lookups:
- If you have an email: `directory_get_user_by_email` using the Black Duck email.
- If you have a username: `directory_get_user` using the Black Duck `userName` / `externalUserName`.

If any identity-directory lookup returns a user, stop — do **not** suggest deactivation.

If no directory record is found:
- Suggest deactivation due to the possibility the user has left the company.

### Step C — Manual confirmation via Slack (required)
directory not-found is not sufficient by itself.
...
Required operations:
1) Confirm identity in the identity directory (Step 1)
...
1) Confirm identity in the identity directory (Step 1) → `{ldap_username, email}`
